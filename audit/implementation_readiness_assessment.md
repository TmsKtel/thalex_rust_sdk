...
- ⚠️ Нет информации о том, как изменить `subscribe_channel()` и места использования подписок
- ⚠️ Нет информации о том, как изменить `resubscribe_all()` для переподписки

**Что нужно добавить:**
- Полный список всех мест, где используется `public_subscriptions` и `private_subscriptions`
- Пошаговая инструкция по замене
- Примеры изменений в `subscribe_channel()`, `unsubscribe()`, `resubscribe_all()`
